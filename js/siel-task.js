...
var NtasksModel;
var percentual;
var arrTasksOfModel =  []; //essa array salva as tarefas da palavra modelo que será retreinada
var retreina = 0; //variavel que conta o numero de vezes do retreino
...
var NtasksRetreino; // Armazena número total de tarefas do retreino corrente
var modo;
var w = [];
...
function confirmExit() { // Exibe tela que confirma se deseja sair da sessão atual
	...
	confirmHtml = confirmHtml.concat("<div class=\"col-lg-6 coluna\">");
	confirmHtml = confirmHtml.concat("<input type=\"image\" src=\"res/thumbs_up.png\" onclick=\"cancelRetreino();playOutro()\" style=\"width: 300px; height: 300px\">");
	confirmHtml = confirmHtml.concat("</div><!-- /.col-lg-6 coluna -->");
...
function loadTasksLocal(repN) { // Gera o HTML das tarefas de um repertório, recebendo o ID numérico desse repertório
	...
	Ntasks = parsedFiles[repN].length;
        ...
        for (var i=0;i<Ntasks;i++) { // Gera o HTML para cada tarefa, passando os dados do repertório
		 modo = parsedFiles[repN][i]["Tipo de Tarefa"];
		 w = [];
...
//essa função recebe como parametro uma palavra e salva no arrPalavra as tarefas na qual essa palavra é modelo,util para o retreino
function getTasksModel(model,repN) {
   // console.log(model);
        if(!inRetreino) {
    for(var i = 0;i < parsedFiles[repN].length;i++) {
        
        if(parsedFiles[repN][i]["Modelo"] == model) {
            arrTasksOfModel.push(parsedFiles[repN][i])  ;
            //console.log(parsedFiles[repN][i]["Modelo"]);
            //console.log(arrTasksOfModel);
        }
    }
    }

} 
function loadTasksRetreino(repN,arrTasks) { // Gera o HTML das tarefas de um repertório, recebendo o ID numérico desse repertório
inRetreino = true;
/*	
    log[repN] = []; // Vetor onde resultados de log serão salvos
	log[repN]["dataSaved"] = true; // Informa que repertório possui dados salvos
	...
	for (var i in arrTasks) { // Gera o HTML para cada tarefa, passando os dados do repertório
		 modo = arrTasks[i]["Tipo de Tarefa"];
                 
		 w = [];
		w.push(arrTasks[i]["Modelo"]);
		for (var j = 1; j <= 10; j++) {
			if (arrTasks[i]["Comp"+j] == "") {
				break;
			}
			w.push(arrTasks[i]["Comp"+j]);
		}
                //tasksHtml[i] = "";
		tasksHtml[i] = generateTaskHtml(repN, i, modo, w);
...
function checkReforco(repN) { // Verifica se tarefa é treino ou não
    ...
    if (reforcoIndex[r][t][0] || reforcoIndex[r][t][1]) {
        ...
        Ntasks = parsedFiles[r].length;
	for (var i=0;i<Ntasks;i++) { // Gera o HTML para cada tarefa, passando os dados do repertório
		var modo = parsedFiles[r][i]["Tipo de Tarefa"];
		var w = [];
		w.push(parsedFiles[r][i]["Modelo"]);
		for (var j = 1; j <= 14; j++) {
			if (parsedFiles[r][i]["Comp"+j] == "") {
				break;
			}
			w.push(parsedFiles[r][i]["Comp"+j]);
		}
                
		tasksHtml[i] = generateTaskHtml(r, i, modo, w);
		// Armazena HTML no div "hidden" invisível, para cache
		hidden.innerHTML = hidden.innerHTML + tasksHtml[i];
		        }
    }
...
function showNextTask() { // Exibe tarefa
    ...
    if (t > Ntasks - 1) { // Se alcançar o número final de tarefas, encerra sessão
       ...
       if (t > NtasksModel - 1) { // Se alcançar o número final de tarefas, encerra sessão
		playOutro();
...
//se nao estiver no retreino,gera as infos iniciais do log
		if(!inRetreino) {
                         // Se tarefa já foi respondida, botões devem parar de funcionar
if (!respostaCerta && !respostaErrada) {
		...
		if (escolha == modelo) { // Se resposta coincidir, resposta correta
			log[r]["t"+t+".a"+log[r]["t"+t+".attempts"]+".result"] = "correto";
			 log[r]["pos_result_num"]++;
                         log[r][""];
            logPalavra.push({"Modelo" : modelo,"Resultado" :log[r]["t"+t+".a"+log[r]["t"+t+".attempts"]+".result"]});

			respostaCerta = true;                 
		} else { // Se não coincidir, resposta errada
			log[r]["t"+t+".a"+log[r]["t"+t+".attempts"]+".result"] = "errado";
			log[r]["neg_result_num"]++;
                        //logPalavra = {Modelo : modelo,Resultado : log[r]["t"+t+".a"+log[r]["t"+t+".attempts"]+".result"]};
       logPalavra.push({"Modelo" : modelo,"Resultado" :log[r]["t"+t+".a"+log[r]["t"+t+".attempts"]+".result"]});
    ...
    //caso esteja em retreino,gera as infos sobre o retreino
else {
...
/*
		...
		log[r]["t"+t+".a"+log[r]["t"+t+".attempts"]+".response"] = escolha; // Salva qual foi a palavra respondida
		*/
               for(var i in arrTasksOfModel) {
                var modelo = arrTasksOfModel[i]["Modelo"];
		var task_type = arrTasksOfModel[i]["Tipo de Tarefa"];
		task_type = task_type.split("");
		if (task_type[1] == "E") { // Se tarefa for do típo sílaba, junte as sílabas da palavra modelo
			modelo = joinWord(modelo);
		}
...
function getResultado (palavra) {
    for(var i in logPalavra) {

    if(logPalavra[i]["Modelo"] == palavra){        
        if(logPalavra[i]["Resultado"] == "correto") {
            acerto ++;
            total ++;
          //console.log("Palavra : " + logPalavra[i]["Modelo"] + "-" + "Resultados Positivos : " + acerto);
...
//essa funcao elabora uma rotina que cancela o retreino caso a criança abandone a sessão antes do fim clicando na portinha
function cancelRetreino () {
    for (var i = 0; i < parsedFiles[r].length; i++) {
        retreinoIndex[r] = false;
    }
}

function retreinar (palavra,repN) {
getResultado(palavra);
getTasksModel(palavra,repN);

    for (var i in arrTasksOfModel) {
      
      if(palavra == arrTasksOfModel[i].Modelo){
            NtasksModel = arrTasksOfModel.length;           
        percentual = (acerto * 100)/NtasksModel;     
    }
    }
        if(percentual < 65) {
