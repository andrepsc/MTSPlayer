...
//var logPalavra = {}; //log da palavra p\retreino
	...
	Papa.parse(file, {
		...
		complete: function (results) { // Ao retornar resultados, armazená-los e atualizar tabela de repertório
			...
			//for (var j = 0; j < repdata.length; j++) {
                       // if(repdata["Tipo de Repertorio"]){}
                       for(var j in parsedFiles){
                       for(var k in parsedFiles[j]) {
              if(parsedFiles[j][k]["Tipo de Repertorio"] == "treino"){
                            rIndex.push([true, true]);
                            retreinoIndex.push(true);
                        } else {
                         
                        rIndex.push([false, false]); // Cada tarefa recebe dois booleanos: reforço tipo imagem e reforço tipo áudio
                        retreinoIndex.push(false);
                            }
                                }
			}   
                       //}
		   ...
		   /*
                    for(var j in parsedFiles) {    
                    for(var k in parsedFiles[j]["Modelo"]) {
                        logPalavra.push([]);
                    }
                }*/
